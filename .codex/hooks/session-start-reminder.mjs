#!/usr/bin/env node
// SessionStart hook for Codex-main sessions (Atlasmith working
...
const ultraContext = [
  "Atlasmith ULTRA fleet mode (CODEX_MODE=ultra) — outcome gates only:",
  ...
  "- USER-FACING LANGUAGE (2026-07-25): the kickoff declaration and every other document the USER is asked to read or approve is written in JAPANESE (code, identifiers, commands stay English). A decision document the decider can't read is not evidence.",
  ...
  "- WAIT DISCIPLINE: wait_agent blocks ONCE with timeout matched to the child's expected duration (600000 measured OK; re-wait if needed). Never poll at 10-30s intervals — each timed-out poll burns a parent reasoning turn. 'Wait timed out' is NOT an error.",
  "- LANDING DISCIPLINE (2026-07-17): a diff that passed its review is COMMITTED before the next phase or investigation starts — never hold reviewed-but-uncommitted work while new investigation children run. CONSULT HARD TRIGGER: never spawn a 3rd child for the SAME symptom — the parent runs the consult window (ask-advisor) first.",
...
const context = [
  "Atlasmith workflow policy (ENFORCED by hooks, not just AGENTS.md):",
  ...
  "- WAIT DISCIPLINE (2026-07-16): wait_agent blocks ONCE with timeout matched to the child's expected duration (timeout=600000 measured OK; re-wait if still running). NEVER poll at 10-30s intervals — every timed-out poll burns a full parent reasoning turn (measured: 826 short waits -> 553 'Wait timed out' turns in one session). 'Wait timed out' is NOT an error; it means the child is still running.",
  "- LANDING DISCIPLINE (2026-07-17): a diff that passed its review is COMMITTED before the next phase or investigation starts. Never hold reviewed-but-uncommitted work while spawning new investigation children (measured: 1,276 reviewed lines held ~24h ran into an EOL incident that forced git-based file reconstruction).",
  "- Stop conditions: after 2 failures of the same approach, change method or consult the non-main AI — never loop. Escalate after 2x refute/rework. HARD TRIGGER: never spawn a 3rd child for the SAME symptom — run the consult window (node ~/.agent-workflow/ask-advisor.mjs) first (measured: one EOL symptom burned ~10 same-shape children while the window went unused for 49h).",
