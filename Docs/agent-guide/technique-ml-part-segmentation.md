...
## SAM2 AMG の反証実験 — SAMesh parity probe(2026-08-01)

> **等級**: 本節の数値・観察はすべて【実測 2026-08-01、オーケストレーター実行】の**転記**である。
> **本節の執筆担当は probe を一度も実行していない**(1つも測っていない)。外部情報は【外部】(URL付き)、
> 数値以外の解釈・機構の説明は【推測】として明示する。

### 経緯

- 単色 peanut_mesh(6016面)に対して SAM2 の既定 AMG が **P=1 に潰れた**(accuracy 0.50 / Δ 0.00)
  【実測 2026-08-01、オーケストレーター実行】。`points_per_side` 8/16/32 × `image_size` 512/1024 の
  直交探索も**全滅**。→ **同一手法2回失敗で停止**し、規約どおり Codex に相談した。
- **Codex 相談の結論**: 同型課題の公開研究 **SAMesh**(arXiv 2408.13679
  【外部 https://arxiv.org/abs/2408.13679 】、実装【外部 https://github.com/gtangg12/samesh 】)が
  「法線+Shape Diameter レンダ・低信頼度閾値(pred_iou 0.5 / stability 0.7)・シルエット内点グリッド・
  hiera-Large」で**無テクスチャメッシュの AMG 部位分割を成立させている**。閾値パラメータは
  「候補を生成後に捨てる」フィルタであり**生成能力を変えない**、という公式実装の読解も提示された。
- **処方**: 閾値0で全候補を取得し、「**filter 前にローブ候補が存在するか**」を停止条件とする
  **1回限りの反証実験**を行う。

### 実測(RTX 4080 / sam2.1-hiera-large / 24視点 / 1024²)【実測 2026-08-01、オーケストレーター実行】

- **filter 前にローブ候補は存在した**(8視点で normal 27 / sdf 30。SAMesh 閾値通過 9 / 16)。
  → **tiny + 既定閾値(0.88/0.95)が候補を捨てていたのが原因**。
- ただし通過候補を全部 fusion に流すと **accuracy 0.5108 / Δ +0.0108** — シルエット全体マスクが
  辺投票でローブ票を溺れさせる(Codex が事前警告した反例と同構図。この機構の説明は【推測】であり、
  実測されたのは数値である)。
- **面積帯フィルタ [0.05, 0.85](シルエット比)を追加した最終測定**:
  **P=2 / accuracy 0.9574 / Δ +0.4574** — 床(0.80 / 0.15)を大幅にクリア。
- **切り分け(決定打)**:

  | チャンネル | P | accuracy | Δ |
  |---|---|---|---|
  | normal 単独 | 1 | 0.5000 | +0.0000(全滅) |
  | **sdf 単独** | **2** | **0.9574** | **+0.4574(複合と完全一致)** |

  → **Shape Diameter が信号のすべて**。単色メッシュでは法線チャンネルは寄与ゼロ。
- **時間**: AMG 約5秒/画像(large)。24視点×2チャンネルで**約4分/メッシュ**。
  sam2 重みロード 3.4秒(キャッシュ済)。

### ユーザー裁定(2026-08-01)

| 内容 | 裁定 |
|---|---|
| Step 2-5 は SAMesh 系設計(モデル既定 sam2.1-hiera-large / AMG 閾値 0.5/0.7 / シルエット内点グリッド / 面積帯フィルタ / normal+SDF の2チャンネル)を基礎に実装 | **承認** |
| モデル既定は large。tiny+新設定の再検証は Step 2-5 の probe で | **承認** |

- **ML ゲート閾値の提案**: accuracy ≥ 0.85 / Δ ≥ 0.30(実測に余裕を持たせつつ、
  床(0.80 / 0.15)より厳格)。

### 計画からの設計変更(計画v4 §2.4.4 との差分)

1. モデル既定 tiny → **sam2.1-hiera-large**(tiny は Step 2-5 の probe で再評価)
2. AMG: 既定閾値 → **0.5 / 0.7 + シルエット内部 point_grids**
3. マスク正規化に**面積帯フィルタ [0.05, 0.85]** を追加
4. 入力画像に **Shape Diameter チャンネルを必須追加**(面ごとレイキャスト。20万面スケールには
   BVH 自前実装が必要 — 依存追加なし)

### 再評価トリガーへの追記

本記録 164 行の再評価トリガーに以下を加える:

- tiny+新設定が床を超えた場合(既定を軽い方へ)
- 実AI生成メッシュ(テクスチャ付き)での実測が取れた場合(shading チャンネルの寄与を再評価)
