...
## 実装タスクの引き継ぎテンプレート(メイン側の実装エージェント委譲用)

**クロスCLIの実装移譲は廃止(2026-07-12 ユーザー決定)** — 実装は各メインが自陣で完結する
(Claude メイン: `implementer` サブエージェント / Codex メイン: spawn_agent の実装者)。
パートナーAIの役割はレビュー二次と相談のみ(→ model-playbook.md の廃止記録)。
以下のテンプレートは**自陣の実装エージェントへのブリーフ**として使う。空欄のまま渡さない。
**ループ禁止条項と代替案報告は削らないこと**(拘泥ループへの補償装置)。
