...
- 対応範囲は**単一メッシュ・単一マテリアル・単一UVセット**に限定する。複数メッシュ GLB 等は
  明確なエラーメッセージで拒否する。
- **重複面(同一頂点集合の面)・非多様体を含むメッシュは非対応**(Phase 1)。
  `_naive_unwrap_and_pack`(`pack/xatlas_naive.py`)の `_build_face_map` は新旧の面対応を
  頂点 index 集合(frozenset)の逆引きで一意に構築するため、同一頂点集合の面が2つ以上ある
  メッシュでは面対応を一意に決められず `ValueError` で停止する(縮退面=頂点 index 重複の面も
  同様に停止)。これは黙って誤った焼き直しを返さないための設計であり、クリーンな単一メッシュ
  (重複面・非多様体の無いもの)を前提とする。AI生成メッシュに重複・両面三角形があると
  rebake 全体が停止しうる。将来のフェーズで重複面の多重度割り当てに対応しうる。
